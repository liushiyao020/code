...
public class Parser {
	...
	AnalyseNode ADD_SUB, DIV_MUL, ADD, SUB, AND, OR, NOT, DIV, MUL, ASS_F, ASS_R, ASS_Q, ASS_U, TRAN_LF;
	...
	public void init() {
		...
		NOT = new AnalyseNode(AnalyseNode.ACTIONSIGN, "@NOT", null);
		...
		SINGLE = new AnalyseNode(AnalyseNode.ACTIONSIGN, "@SINGLE", null);//单目运算符
	...
	private void nonTermOP(String nonTerm) {
		...
		case '5':// l'
			if (firstWord.value.equals("+")) {
				...
				analyseStack.add(0, new AnalyseNode(AnalyseNode.TERMINAL, "&&", null));
				analyseStack.add(1, L);
				analyseStack.add(2, AND);
			} else if (firstWord.value.equals("||")) {
				analyseStack.remove(0);
				analyseStack.add(0, new AnalyseNode(AnalyseNode.TERMINAL, "||", null));
				analyseStack.add(1, L);
				analyseStack.add(2, OR);
			} else if (firstWord.value.equals("!")) {
				analyseStack.remove(0);
				analyseStack.add(0, new AnalyseNode(AnalyseNode.TERMINAL, "!", null));
				analyseStack.add(1, L);
				analyseStack.add(2, NOT);
			} else {
				analyseStack.remove(0);
			}
			break;
	...
	private void actionSignOP() {
		if (top.name.equals("@ADD_SUB")) {
			...
			OP = "-";
			analyseStack.remove(0);
		} else if (top.name.equals("@AND")) {
			OP = "&&";
			analyseStack.remove(0);
		} else if (top.name.equals("@OR")) {
			OP = "||";
			analyseStack.remove(0);
		} else if (top.name.equals("@NOT")) {
			OP = "!";
			analyseStack.remove(0);
...
//			System.out.println(fourElemCount + top.name);
